...
class StreamMonitor:
        ...
        try:
            ...
            if stream_url != getattr(self, "_last_analyzed_url", None):
                self.stream_logger.info(
                    f"New stream URL detected, analyzing: {stream_url[:50]}..."
                )
                ...
                if stream_info:
                    ...
                    self.stream_logger.info(
                        f"Stream analysis successful: {stream_info.get('format_name', 'unknown')} format"
                    )
                else:
                    self.stream_availability.set(0)
                    self.stream_logger.warning(
                        f"Stream analysis failed (URL may have expired): {stream_url[:50]}..."
                    )
                    # Keep previous metrics for a while in case of temporary issues
            else:
                # Same URL, just update availability
                self.stream_availability.set(1)
                self.stream_logger.debug(
                    "Stream URL unchanged, maintaining current metrics"
                )
        ...
        if self.process_manager:
            try:
                ...
                if process_info and "url" in process_info:
                    self.stream_logger.debug(
                        f"Got stream URL from process manager: {process_info['url']}"
                    )
                    return process_info["url"]
            except Exception as e:
                self.stream_logger.error(
                    f"Failed to get stream URL from process manager: {e}"
                )
    ...
    def _analyze_stream(self, url: str) -> dict | None:
        ...
        try:
            ...
            try:
                ...
                if process.returncode != 0:
                    self.stream_logger.error(
                        f"ffprobe failed with return code {process.returncode}: {stderr}"
                    )
                    return None
                ...
                process.kill()
                self.stream_logger.error(
                    f"ffprobe timed out after {self.ffprobe_timeout} seconds"
                )
                return None
        except Exception as e:
    ...
    def _update_stream_metrics(self, stream_info: dict):
        ...
        try:
            ...
            self.stream_logger.error(f"Error updating stream metrics: {e}")
