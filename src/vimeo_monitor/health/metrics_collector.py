...
class MetricsCollector:
    ...
    def initialize(self):
        ...
        if initialized_count == 0:
            ...
            self.metrics_logger.info(
                f"Successfully initialized {initialized_count} health collectors"
            )
    ...
    def _initialize_monitors(self):
        ...
        if self.monitor:
            try:
                ...
                self.metrics_logger.error(
                    f"Unexpected error initializing script monitor: {e}"
                )
        ...
        if getattr(self.config, "health_hardware_enabled", False):
            try:
                ...
                self.system_monitor = SystemMonitor(
                    config=self.config, logger=self.logger, registry=self.registry
                )
                ...
                self.metrics_logger.error(
                    f"Unexpected error initializing system monitor: {e}"
                )
        else:
        ...
        if getattr(self.config, "health_network_enabled", False):
            try:
                ...
                self.metrics_logger.error(
                    f"Unexpected error initializing network monitor: {e}"
                )
        else:
        ...
        if getattr(self.config, "health_stream_enabled", False):
            try:
                ...
                self.metrics_logger.error(
                    f"Unexpected error initializing stream monitor: {e}"
                )
        else:
    ...
    def _start_collection_threads(self):
        ...
        if self.script_monitor:
            ...
            self._start_collection_thread(
                "script", self.script_monitor.update_metrics, interval
            )
            thread_count += 1
        ...
        if self.system_monitor:
            ...
            self._start_collection_thread(
                "system", self.system_monitor.update_metrics, interval
            )
            thread_count += 1
        ...
        if self.network_monitor:
            ...
            self._start_collection_thread(
                "network", self.network_monitor.update_metrics, interval
            )
            thread_count += 1
        ...
        if self.stream_monitor:
            ...
            self._start_collection_thread(
                "stream", self.stream_monitor.update_metrics, interval
            )
            thread_count += 1
    ...
    def _start_collection_thread(self, name: str, collection_func, interval: int):
        ...
        thread = threading.Thread(
            target=collection_loop, name=f"metrics-{name}", daemon=True
        )
        thread.start()
        self.collection_threads[name] = thread
        self.metrics_logger.info(
            f"Started {name} metrics collection thread (interval: {interval}s)"
        )
    ...
    def shutdown(self):
        ...
        if not self.running:
            self.metrics_logger.debug(
                "Metrics collection not running, nothing to shut down"
            )
            return
        ...
        for name, thread in self.collection_threads.items():
            if thread.is_alive():
                self.metrics_logger.info(
                    f"Waiting for {name} metrics collection thread to terminate"
                )
                thread.join(timeout=1.0)
                if thread.is_alive():
                    self.metrics_logger.warning(
                        f"{name} metrics collection thread did not terminate"
                    )
                else:
                    self.metrics_logger.info(
                        f"{name} metrics collection thread terminated"
                    )
