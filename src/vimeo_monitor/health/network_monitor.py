...
class NetworkMonitor:
    ...
    def __init__(self, config: Config, logger: Logger, registry=None):
        ...
        self.speedtest_enabled = (
            getattr(self.config, "health_network_speedtest_enabled", False)
            and SPEEDTEST_AVAILABLE
        )
        ...
        # Speedtest interval
        self.speedtest_interval = getattr(
            self.config, "health_network_speedtest_interval", 300
        )
        self.last_speedtest_time = 0

        # Ping hosts
        self.ping_hosts = getattr(
            self.config,
            "health_network_ping_hosts",
            ["8.8.8.8", "1.1.1.1", "vimeo.com"],
        )
    ...
    def update_metrics(self):
        ...
        try:
            ...
            if (
                self.speedtest_enabled
                and (current_time - self.last_speedtest_time) >= self.speedtest_interval
            ):
            ...
            try:
                ...
                if latency is not None:
                    ...
                    self.network_logger.debug(
                        f"Host {host} is reachable with latency {latency:.2f}ms"
                    )
                else:
    ...
    def _ping_host(self, host: str) -> float | None:
    ...
    def _update_dns_metrics(self):
        ...
        for domain in domains:
            try:
                ...
                self.network_logger.debug(
                    f"DNS resolution time for {domain}: {resolution_time:.2f}ms"
                )
            except Exception as e:
