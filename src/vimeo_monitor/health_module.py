...
class HealthModule:
    ...
    def _log_configuration(self):
        ...
        self.health_logger.info(f"Health metrics host: {self.config.health_metrics_host}")
        self.health_logger.info(f"Health metrics port: {self.config.health_metrics_port}")
        
        # Log enabled collectors
        enabled_collectors = []
        if hasattr(self.config, "health_hardware_enabled") and self.config.health_hardware_enabled:
            enabled_collectors.append("Hardware")
            self.health_logger.info(f"Hardware monitoring interval: {self.config.health_hardware_interval}s")
        
        if hasattr(self.config, "health_network_enabled") and self.config.health_network_enabled:
            enabled_collectors.append("Network")
            self.health_logger.info(f"Network monitoring interval: {self.config.health_network_interval}s")
            if hasattr(self.config, "health_network_speedtest_enabled") and self.config.health_network_speedtest_enabled:
                self.health_logger.info(f"Network speedtest enabled (interval: {self.config.health_network_speedtest_interval}s)")
                self.health_logger.info(f"Network ping hosts: {self.config.health_network_ping_hosts}")
        
        if hasattr(self.config, "health_stream_enabled") and self.config.health_stream_enabled:
            enabled_collectors.append("Stream")
            self.health_logger.info(f"Stream monitoring interval: {self.config.health_stream_interval}s")
            self.health_logger.info(f"FFprobe timeout: {self.config.health_stream_ffprobe_timeout}s")
    ...
    def _run_server(self):
        ...
        try:
            self.health_logger.info(f"Starting FastAPI server on {self.config.health_metrics_host}:{self.config.health_metrics_port}")
            uvicorn.run(
                self.app,
    ...
    def shutdown(self):
        ...
        if not self.running:
            self.health_logger.debug("Health monitoring not running, nothing to shut down")
            return
        ...
        if self.server_thread and self.server_thread.is_alive():
            ...
            if self.server_thread.is_alive():
                self.health_logger.warning("Server thread did not terminate within timeout")
            else:
