...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.health_monitoring_enabled: bool = self._get_bool(
        ...
        if self.health_monitoring_enabled:
            ...
            self.health_metrics_port: int = int(
                os.getenv("HEALTH_METRICS_PORT", "8080")
            )
            ...
            self.health_stream_interval: int = int(
                os.getenv("HEALTH_STREAM_INTERVAL", "60")
            )

            # Feature toggles
            self.health_hardware_enabled: bool = self._get_bool(
                "HEALTH_HARDWARE_ENABLED", True
            )
            self.health_network_enabled: bool = self._get_bool(
                "HEALTH_NETWORK_ENABLED", True
            )
            self.health_stream_enabled: bool = self._get_bool(
                "HEALTH_STREAM_ENABLED", True
            )

            # Network monitoring configuration
            ping_hosts = os.getenv(
                "HEALTH_NETWORK_PING_HOSTS", "8.8.8.8,1.1.1.1,vimeo.com"
            )
            self.health_network_ping_hosts: list[str] = [
                h.strip() for h in ping_hosts.split(",")
            ]
            self.health_network_speedtest_enabled: bool = self._get_bool(
                "HEALTH_NETWORK_SPEEDTEST_ENABLED", True
            )
            self.health_network_speedtest_interval: int = int(
                os.getenv("HEALTH_NETWORK_SPEEDTEST_INTERVAL", "300")
            )

            # Stream monitoring configuration
            self.health_stream_ffprobe_timeout: int = int(
                os.getenv("HEALTH_STREAM_FFPROBE_TIMEOUT", "15")
            )
    ...
    def validate(self) -> None:
        ...
        if self.health_monitoring_enabled:
            ...
            if self.health_network_speedtest_interval < 60:
                raise ValueError(
                    "Health network speedtest interval must be at least 60 seconds"
                )

            if self.health_stream_ffprobe_timeout < 1:
                raise ValueError(
                    "Health stream FFprobe timeout must be at least 1 second"
                )
